 ...
 */
@WebServlet("/ReplaceMessage")
public class ReplaceMessage extends HttpServlet 
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String messageText 	= paramMap.get("messageText");
		
		if ( senderID == null || recipiID == null || messageID == null || messageText == null )
		{
		...
		{			
			...
			try
			{
				...
				historyManager.replaceMessagesFromHistory( senderID, recipiID, mID, new HistoryManager.ReplaceEntryEvent()
				{
					...
					{
						ChatMessageCore oldChatMessage = (ChatMessageCore)JsonReader.jsonToJava( entry );

						TextMsg	txtMessage = new TextMsg( 0, messageText );

						txtMessage.setChatContentClass( "quick_chat.adapters.chat.TextMessage" );

						ChatMessageCore newChatMessage = new ChatMessageCore( 	txtMessage,
																				MessageType.kSingleUser,
																				oldChatMessage.getSenderName() ,
																				oldChatMessage.getSenderID(),
																				MessageType.kSingleUser,
																				oldChatMessage.getReceiverName(),
																				oldChatMessage.getReceiverID(),
																				oldChatMessage.getTime() );
						
						newChatMessage.setId(mID);
						
						chatMessageArr[0] = newChatMessage;
						
						String jsonStr = JsonWriter.objectToJson( newChatMessage );
